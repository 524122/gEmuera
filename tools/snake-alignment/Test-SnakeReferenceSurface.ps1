...
$behaviorChecks = @(
    ...
    @{ Path = (Join-Path $project 'Scripts\Emuera\GameView\EmueraConsole.cs'); Pattern = '(?ms)inputs\.IndexOf\("\\\\e"\)\s*>=\s*0.*?inputs\s*=\s*inputs\.Replace\("\\\\e",\s*""\).*?MesSkip\s*=\s*true'; Label = 'SEQUENCEINPUT \e MesSkip consumption' },
    @{ Path = (Join-Path $project 'Scripts\Emuera\GameView\EmueraConsole.cs'); Pattern = '(?ms)if\s*\(!inputMacroEnabled\).*?callEmueraProgram\(str,\s*changedByMouse\)\s*;.*?goto\s+endMacro\s*;'; Label = 'disabled input macro literal bypass' },
    ...
    @{ Path = (Join-Path $project 'Scripts\Emuera\GameProc\Function\ArgumentBuilder.cs'); Pattern = 'class\s+SP_COLOR_ALPHA_ArgumentBuilder'; Label = 'TEXT_BGC_ON strict argument builder' },
