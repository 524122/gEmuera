...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'docs\NewFrameworkDesign\generated\dialect-policy-consumer-boundary.json' }
...
try {
    ...
    $vocabulary = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'docs\NewFrameworkDesign\generated\dialect-declaration-vocabulary.json') | ConvertFrom-Json
