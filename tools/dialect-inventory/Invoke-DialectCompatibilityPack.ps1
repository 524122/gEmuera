...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'NewFrameworkDesign\generated\dialect-compatibility-pack.json' }

try {
    Import-Module (Join-Path $scriptRoot 'DialectCompatibilityPack.psm1') -Force
    $planPreflight = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'NewFrameworkDesign\generated\dialect-plan-preflight.json') | ConvertFrom-Json
    $profileSelection = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'NewFrameworkDesign\generated\dialect-profile-selection.json') | ConvertFrom-Json
