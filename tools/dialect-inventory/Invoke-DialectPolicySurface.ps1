...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'NewFrameworkDesign\generated\dialect-policy-surface.json' }

try {
    Import-Module (Join-Path $scriptRoot 'DialectPolicySurface.psm1') -Force
    $boundary = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'NewFrameworkDesign\generated\dialect-policy-consumer-boundary.json') | ConvertFrom-Json
