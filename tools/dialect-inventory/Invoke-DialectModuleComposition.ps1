...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'NewFrameworkDesign\generated\dialect-module-composition.json' }

try {
    Import-Module (Join-Path $scriptRoot 'DialectModuleComposition.psm1') -Force
    $compatibilityPack = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'NewFrameworkDesign\generated\dialect-compatibility-pack.json') | ConvertFrom-Json
    $policySurface = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'NewFrameworkDesign\generated\dialect-policy-surface.json') | ConvertFrom-Json
