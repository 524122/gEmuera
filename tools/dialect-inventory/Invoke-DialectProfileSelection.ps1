...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'NewFrameworkDesign\generated\dialect-profile-selection.json' }

try {
    Import-Module (Join-Path $scriptRoot 'DialectProfileSelection.psm1') -Force
    $plan = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $project 'NewFrameworkDesign\generated\dialect-plan-preflight.json') | ConvertFrom-Json
