...
namespace MinorShift.Emuera.Content
{
	internal sealed class GraphicsImage : AbstractImage
	{
		...
		/// アルファ値がalphaThreshold以下の画素を一括で0x0(透明黒)にする。
		/// ERB側の画像_僅少アルファ除去はFOR二重ループでGGETCOLOR/GSETCOLORを
		/// 画素ごとに呼んでおり、Image.GetPixel/SetPixelのネイティブ境界チェックと
		/// MarkImageMutatedの呼び出しコストが立ち絵サイズ分(数十万回)積み重なって
		/// Android端末でスクリプトスレッドを止める要因になっていた。
		/// ApplyColorMatrix(340行目付近)と同じGetData/SetData一括処理パターンを踏襲し、
		/// ロック内で1回のバイト配列読み書きに畳み込む。
		/// エラーチェックは呼び出し元でのみ行う
		/// </summary>
