...
namespace MinorShift.Emuera
{
	...
	internal static class Config
	{
		...
		private static long getUpdateKey()
		{
			...
			var erbFiles = uEmuera.Utils.GetFilePaths(Program.ErbDir, "*.ERB", option);
			var csvFiles = uEmuera.Utils.GetFilePaths(Program.CsvDir, "*.CSV", option);
			long[] writetimes = new long[erbFiles.Count + csvFiles.Count];
			for (int i = 0; i < erbFiles.Count; i++)
				if (Path.GetExtension(erbFiles[i]).Equals(".ERB", StringComparison.OrdinalIgnoreCase))
					writetimes[i] = uEmuera.Utils.GetLastWriteTimeKey(erbFiles[i]);
			for (int i = 0; i < csvFiles.Count; i++)
				if (Path.GetExtension(csvFiles[i]).Equals(".CSV", StringComparison.OrdinalIgnoreCase))
					writetimes[i + erbFiles.Count] = uEmuera.Utils.GetLastWriteTimeKey(csvFiles[i]);
			long key = 0;
		...
		public static void UpdateWindowWidth(int width)
		{
			WindowX = width;
			DrawableWidth = width - DrawingParam_ShapePositionShift;
		}
