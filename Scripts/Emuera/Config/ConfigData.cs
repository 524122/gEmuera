...
#region getitem
		...
		public bool LoadConfig()
		{
			...
			if(!uEmuera.Utils.FileExists(defaultConfigPath))
		...
		{
			if (!uEmuera.Utils.FileExists(confPath))
				return false;
			...
			try
			{
				...
				while ((line = eReader.ReadLine()) != null)
				{
					...
					if (string.IsNullOrWhiteSpace(line))
						continue;
					if (md5i >= keyOnlyMd5s.Count)
					{
						GenericUtils.Warn($"[CONFIG] md5 index overflow at line {lineCount}: '{line}'");
						configDebugLog.AppendLine($"  MD5 OVERFLOW at line {lineCount}: '{line}'");
						break;
					}
					var md5 = keyOnlyMd5s[md5i++];
					if (line[0] == ';')
						continue;
					...
					if (item == null)
					{
						...
						GenericUtils.Warn($"[CONFIG] Unmatched key: '{token_0}'");
					}
					else
					{
						configDebugLog.AppendLine($"  OK: {item.Code} = '{tokens[1].Trim()}'");
					}
					if (item != null)
					{
						//1806beta001 CompatiDRAWLINEの廃止、CompatiLinefeedAs1739へ移行
			...
			{
				GenericUtils.Warn($"[CONFIG] EmueraException at line {lineCount}: {ee.Message}");
				ParserMediator.ConfigWarn(ee.Message, pos, 1, null);
			}
			catch (Exception exc)
			{
				GenericUtils.Error($"[CONFIG] Exception at line {lineCount}: {exc.GetType().Name}: {exc.Message}");
		...
		/// <summary>
		/// Calculate MD5s for just the key portion of each line (before ':'),
		/// matching the format of emuera_config_shiftjis.bytes which has key-only lines.
		/// </summary>
		private static List<string> CalcKeyOnlyMd5List(byte[] bytes)
		{
			...
			for (int i = 0; i < bytes.Length; i++)
			{
				if (bytes[i] == 0x0A)
				{
					ExtractKeyBytes(bytes, start, i, lines);
					start = i + 1;
				}
			}
			if (start < bytes.Length)
				ExtractKeyBytes(bytes, start, bytes.Length, lines);
		...
		private static void ExtractKeyBytes(byte[] bytes, int start, int end, List<byte[]> lines)
		{
			int len = end - start;
			if (len > 0 && bytes[end - 1] == 0x0D)
				len--;
			if (len <= 0)
				return;
			// Find the key part (before first ':')
			int keyLen = len;
			for (int j = 0; j < len; j++)
			{
				if (bytes[start + j] == 0x3A)
				{
					keyLen = j;
					break;
				}
			...
			while (keyLen > 0 && (bytes[start + keyLen - 1] == 0x20 || bytes[start + keyLen - 1] == 0x09))
				keyLen--;
			if (keyLen <= 0)
				return;
			var lineBytes = new byte[keyLen];
			Array.Copy(bytes, start, lineBytes, 0, keyLen);
			lines.Add(lineBytes);
		}
...
#region debug
		...
		{
			if (!uEmuera.Utils.FileExists(configdebugPath))
				goto err;
			EraStreamReader eReader = new EraStreamReader(false);
