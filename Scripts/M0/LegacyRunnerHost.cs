    ...
    public partial class LegacyRunnerHost : Node
    {
        ...
        public override void _Ready()
        {
            try
            {
                ...
                var launchRegistry = _config.CreateM0RunnerSessionLaunchRegistry();
                ...
                if (!global::FirstWindow.ConfigureM0RunnerSession(_config.GameRoot, _config.Profile, out string error))
                    throw new InvalidDataException(error);
                global::MinorShift.Emuera.Program.ConfigureM0RunnerStartupErrorLogPath(
                    Path.Combine(_config.OutputDirectory, "emuera_startup_errors.log"));
                global::MinorShift.Emuera.Program.ConfigureM0RunnerDefaultOutputLogPath(
                    Path.Combine(_config.OutputDirectory, "emuera.log"));
                if (!global::EmueraContent.ConfigureM0RunnerDisplayBackend(_config.DisplayBackend, out error))
                    throw new InvalidDataException(error);

                EnsureM0ViewportSize();
                ...
                _main.ConfigureM0RunnerSessionLaunchRegistry(launchRegistry);
        ...
        void SettleInProcessSessionCycle(MinorShift.Emuera.GameView.EmueraConsole console, long elapsedMs)
        {
            ...
            if (_inProcessWaitFingerprints.Count < _inProcessSessionCycleTargets.Count)
            {
                ...
                _pendingInProcessSessionSwitch = _main.SwitchLegacySessionForM0RunnerAsync(nextTarget);
                return;
            }
        ...
        static string CaptureSettlementFingerprint(MinorShift.Emuera.GameView.EmueraConsole console)
        {
            ...
            string surface = global::EmueraContent.instance == null
                ? "surface_unavailable"
                : global::EmueraContent.instance.CaptureM0SettlementFingerprint();
            ...
            MinorShift.Emuera.GameView.EmueraConsole console)
        {
            try
            {
                ...
                if (_config.CaptureScreenshot)
                {
                    ...
                    else
                    {
                        ...
                        screenshot = CaptureM0Screenshot();
                    }
                }
                ...
                LegacyDisplayObservation displayObservation = global::EmueraContent.instance == null
                    ...
                    : global::EmueraContent.instance.CaptureM0LegacyDisplayObservation(_config.DisplayBackend);
            ...
            {
                GD.PushError("M0 legacy runner report failure: " + ex);
                exitCode = 74;
            }
            GetTree().Quit(exitCode);
        }

        LegacyScreenshotEvidence CaptureM0Screenshot()
        {
