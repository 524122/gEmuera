...
- 参考实现：`E:\MyCode\Era\emuera_lazyloading_selfmodified_version-main-skiasharp`、`E:\MyCode\GodotCode\gemuera\uEmuera-0.2.9d`、`E:\MyCode\GodotCode\gemuera\XEmuera-0.5.1等`。
...
| 文档                                                | 状态       | 说明                                                                     |
| ------------------------------------------------- | -------- | ---------------------------------------------------------------------- |
| `docs/gEmueraCodeWiki/00-How-to-Use-This-Wiki.md` | **必读**   | 当前代码地图，减少无目的文件检索；改源码后判断是否需同步更新                                         |
| `docs/xEmueraCodeWiki`                            | 外部参考     | XEmuera-R：一款专门为Emuera1824+v24+EE+EM适配的模拟。这是使用gpt5.6-sol读取其代码并编写的架构指导文档 |
| `docs/NewFrameworkDesign/DeveloperHandoff.md`     | **接手必读** | 当前已有能力、实际 owner、阶段阻断、验证入口和下一开发顺序                                       |
| `docs/NewFrameworkDesign/`                        | 当前设计     | 新架构设计文档（Godot-master skill + 学习 XEmuera 架构重新设计）                        |
| `docs/OriginalFrameworkDesign/`                   | 已过时      | 早期架构设计，部分内容有误，仅供参考                                                     |
...
不要靠猜测定位文件，先查 `docs/gEmueraCodeWiki/README.md` 与 `10-Source-Index.md`；如果当前工作区存在 `CODE_MAP.md`，再按其规则同步更新。
