...
Godot 图形 API、控制台显示后端和未来 DTO renderer 是三个独立决策。当前低端 Android 生产基线在 `project.godot` 固定为 Compatibility/OpenGL ES 3：`renderer/rendering_method` 与 `renderer/rendering_method.mobile` 都是 `gl_compatibility`。它适合本项目的 2D、文本密集界面，并扩大旧 GPU 或 Vulkan/RenderingDevice 不可用设备的覆盖范围。

该选择只在引擎启动前生效，不是按游戏、会话、存档或脚本切换的能力。`EmueraContent` 的 Canvas/Controls 选择仍由其自身 owner 管理；M2/M6 的 DTO 或候选 renderer 仍需各自的 feature flag、golden、性能报告和回退，不能借由 OpenGL 切换获得放行。

Compatibility 可以改善 GPU 驱动、pipeline 或图形基线成本，不能自动减少 ERB 执行、显示 batch、Canvas draw/hit rebuild、Control fallback、图片 decode/upload 或历史保留的成本。TW 起床等高输出流程必须在相同 release APK、fixture、存档状态和输入序列下，用 VM、显示、GPU 与内存分项数据归因；详细协议见 [RendererDeploymentPolicy](RendererDeploymentPolicy.md)。
...
| A-011 | 低端 Android 发布基线使用 Compatibility/OpenGL ES 3；图形 API、控制台 Canvas/Controls 与 M2/M6 renderer 实验分离 | Accepted，设备性能证据待补 |
