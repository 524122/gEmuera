...
| Godot 工程 | Godot 4.7 Mono、桌面 `net8.0`、Android 条件目标 `net9.0`、Compatibility/OpenGL ES 3 renderer、启动器和主场景 | Android 可构建声明不等于当前 APK/真机门禁通过；TW 高输出性能仍需设备归因 |
...
现有 legacy 引擎已承载 `.ERB`/`.CSV` 加载、表达式和变量、指令执行、存档、HTML/图片/形状、按钮输入、声音、SQLite/Map/XML/DataTable 扩展以及 v24/Snake 兼容分支。实际公开功能概览见 [项目 README](../../readme/README.md)；代码定位优先使用本 Wiki，若工作区存在 `CODE_MAP.md` 再按其规则补充。
...
[`src/Core/GEmuera.Core.csproj`](../../src/Core/GEmuera.Core.csproj) 是不引用 GodotSharp 的独立程序集，面向 Godot host 使用 `net8.0`，本地合同验证还提供 `net9.0`。当前目录已经具备以下边界：
...
`BuiltInDialectCatalog.CreateLegacySessionPlan` 现在为 `v24pure`、`snake` 与 `erafl` 创建唯一的冻结 plan；`EmueraMain` 在 legacy console 和 worker 创建前绑定该 plan。`Scripts/Emuera/Compatibility/LegacyCompatibilityProfile.cs` 验证 module closure，并投影为 Snake 与 eraFL 的窄、不可变策略。

`FunctionIdentifier` 与 `FunctionMethodCreator` 保留 legacy handler 作为行为 owner，但 `IdentifierDictionary` 只获得当前 profile 的只读 instruction/function surface。Snake-only instruction 和 `陥落状態` / `陷落状态` 不会进入 v24 或 eraFL lookup。parser、VM、资源、view 和 input 的 profile 差异统一读取 `Program.Compatibility`；`Program.IsSnakeProfile` 与 `Program.IsEraFlProfile` 不再是 `Program.cs` 外的运行期消费点。

这只关闭了主路径的组合/可见性/策略耦合，**不**表示 Core 接管 legacy Parser/VM，也不表示 DIA ownership、真实游戏 fixture、A/B/A、APK 或 Android 真机证据已经通过。Core smoke 与文本架构守卫已覆盖该边界；对应行为与设备门禁仍保持原有 `Blocked` / `Uncovered` 状态。
