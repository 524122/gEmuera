...
## 当前 legacy bridge 的已落地边界

截至 2026-07-23，默认 legacy 路径已完成一层受限的生产隔离；它不是 Core Parser/VM 的迁移完成声明。

| 边界 | 已落地实现 | 仍未完成 / 不可推断 |
| --- | --- | --- |
| session 选择 | `BuiltInDialectCatalog.CreateLegacySessionPlan` 只按 allowlisted `v24pure`、`snake`、`erafl` 构造并冻结 `CompatibilityPlan`；`EmueraMain` 在创建 legacy console 前绑定它。 | legacy VM 仍为进程级，不能同时运行多个不同 profile 的 VM。 |
| module closure | `LegacyCompatibilityProfile` 验证 `gemuera.v24` 基线及 `game.snake` / `game.erafl` 的互斥闭包，再投影为不可变 Snake 和 eraFL 窄策略。 | 这不是游戏内容 resolver，也不裁决 DIA ownership 或 module provenance。 |
| instruction/function surface | `FunctionIdentifier` 和 `FunctionMethodCreator` 从同一 legacy handler store 生成 profile-scoped、只读 lookup；`IdentifierDictionary` 只接收当前 profile 的表。v24 和 eraFL 不再能解析 Snake-only instruction 或 `陥落状態` / `陷落状态`。 | handler 本体仍是 legacy owner；静态表面隔离不等于所有 ERB 行为 fixture 已对齐。 |
| runtime policy | parser、CALL 参数、private argument、warning、resource lazy index、display refresh/history 与 pointer input 通过 `Program.Compatibility` 的窄策略消费。eraFL Core 算法仅在 eraFL policy gate 后可达。 | 未选择的模块不可见不证明复杂 eraFL 地图、显示或输入场景已在真游戏和 Android 上通过。 |
| regression boundary | `CoreContractSmoke` 覆盖三种 profile 的 closure 与 registry visibility；`Test-CoreArchitecture.ps1` 禁止 `Program.IsSnakeProfile` / `Program.IsEraFlProfile` 被 `Program.cs` 外的 legacy 代码重新读取。 | smoke 和文本守卫不能替代两侧 ERB fixture、trace、A/B/A、APK 或设备门禁。 |

因此，旧 `Program` 的公开只读 profile 属性仍只是兼容 adapter；新增 legacy 代码必须消费 `Program.Compatibility` 的具体窄策略，不能重新添加 `if (Snake)`、`if (eraFL)` 或从 registry 的完整 handler store 取得未过滤表。
