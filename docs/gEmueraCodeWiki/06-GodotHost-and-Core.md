...
M2 起，第 1 层的工具面板（`Inputpad`/`Scalepad`/`QuickButtons`/`OptionWindow`）被组件化为 `scenes/*.tscn` 场景资产，由 `EmueraContent` 实例化挂载；`RuntimeDiagnosticsPanel` 也有 `scenes/RuntimeDiagnosticsPanel.tscn` 资产但当前仍由 `EmueraMain` 通过 `AttachFloatingTo` 代码挂载（场景资产仅供复用）。它们仍是 legacy host/presentation 的行为 owner，不属于 GodotHost 桥，也不改变 Core 合同边界。
