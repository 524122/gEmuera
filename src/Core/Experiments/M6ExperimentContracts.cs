...
/// </summary>
public readonly record struct M6FeatureFlags(
    bool Scheduler,
    bool Renderer,
    bool DualRun,
    bool Accessibility)
{
    public static M6FeatureFlags Default => new(
        Scheduler: false,
        Renderer: false,
        DualRun: false,
        Accessibility: true);
...
/// </summary>
public sealed record M6ExperimentIdentity
{
    public M6ExperimentIdentity(
        string experimentId,
    ...
    public M6FeatureFlags Flags { get; }
...
public readonly record struct M6PerformanceThresholds
{
    public M6PerformanceThresholds(
...
/// </summary>
public sealed record M6ExperimentDefinition
{
    public M6ExperimentDefinition(
        M6ExperimentIdentity identity,
        string hypothesis,
        string baselineArtifact,
        string rollbackArtifact,
        M6PerformanceThresholds thresholds,
    ...
    public M6ExperimentIdentity Identity { get; }
    public string Hypothesis { get; }
    public string BaselineArtifact { get; }
    public string RollbackArtifact { get; }
    public M6PerformanceThresholds Thresholds { get; }
    public YieldabilityAuditInventory YieldabilityAudit { get; }
    public M6EvidenceStatus EvidenceStatus => M6EvidenceStatus.ExperimentalOnly;
...
public sealed record M6MetricSample
{
    public M6MetricSample(
...
public sealed record M6MetricPercentiles
{
    internal M6MetricPercentiles(
        string experimentIdentityHash,
        string metricName,
        int sampleCount,
        double p50,
...
/// </summary>
public sealed class M6MetricSeries
{
    private readonly IReadOnlyList<M6MetricSample> _samples;

    public M6MetricSeries(IEnumerable<M6MetricSample> samples)
    {
        ...
        _samples = new ReadOnlyCollection<M6MetricSample>(copy);
    ...
    public string MetricName { get; }
    public IReadOnlyList<M6MetricSample> Samples => _samples;

    public M6MetricPercentiles Summarize()
    {
        var values = _samples.Select(sample => sample.Value).OrderBy(value => value).ToArray();
        return new M6MetricPercentiles(
            IdentityHash,
            MetricName,
            values.Length,
...
public sealed record M6CooperativeVmStep
{
    internal M6CooperativeVmStep(
        M6ExperimentIdentity identity,
        long traceOrdinal,
        VmStepResult result)
    {
...
/// </summary>
public sealed record M6DualRunContract
{
    public M6DualRunContract(M6ExperimentIdentity baseline, M6ExperimentIdentity candidate)
    {
    ...
    public M6ExperimentIdentity Baseline { get; }
    public M6ExperimentIdentity Candidate { get; }
}
