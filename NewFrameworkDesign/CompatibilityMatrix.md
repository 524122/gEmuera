...
| DIALECT.RESOLUTION | 方言/启动器 | 目录路由 profile id、受信任 allowlist 与 legacy profile 投影 | `FirstWindow` 固定枚举 `<root>/<game>`、`<root>/snake/<game>`、`<root>/compat/<profile>/<game>`；compat profile 必须小写并通过 `BuiltInDialectCatalog` allowlist，`v24pure`/`snake` 保留 lane 与未知项 fail closed；`LauncherGameEntry` 将 path/profile/source 直接交给启动链，`Program.DetectCoreProfile` 不再读取 marker 改写选择 | Android 目录矩阵、单游戏高级覆盖、future profile 的 legacy bridge 显式支持 | Partial | Host 实现和桌面编译已通过；仍缺 v24/Snake/eraFL 实际启动、Android 外部存储/重启、未知目录运行时矩阵，且未来新 profile 需同步提供 legacy bridge 映射，不能静默回退 |
